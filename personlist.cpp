...
void personList::mousePressEvent(QMouseEvent *event)
{
    ...
    if(event->button()==Qt::LeftButton && currentItem!=NULL && currentItem==groupMap.value(currentItem))//如果点击的左键并且是点击的是组
    {
        if(isHideMap.value(currentItem))                                  //如果先前是隐藏，则显示
        {
            ...
            currentItem->setIcon(QIcon(":/Resources/LoginWindow/arrowDown.png"));
        }
        else                                                             //否则，先前是显示，则隐藏
        {
            foreach(QListWidgetItem* subItem, groupMap.keys(currentItem))//遍历组的对应的项（包括自身和好友）
             ...
             currentItem->setIcon(QIcon(":/Resources/LoginWindow/arrowRight.png"));
        }
    }
}
...
{
    QListWidgetItem *newItem=new QListWidgetItem(QIcon(":/Resources/LoginWindow/arrowRight.png"),"未命名");    //创建一个Item
...
void personList ::loadStyleSheet(const QString &sheetName)
{
    QFile file(":/Resources/LoginWindow/" + sheetName + ".css");
